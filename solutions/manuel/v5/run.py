...
from cgiar.model import TIMM_Model
from cgiar.utils import get_dir, time_activity
from cgiar.data import CGIARDatasetV2, augmentations
...
def run():
    ...
    OUTPUT_DIR=get_dir('solutions/manuel/v5', args.subfolder)
    ...
    with torch.no_grad():
        for ids, images, _ in test_loader:
            outputs = torch.softmax(model(images.to(device)), dim=-1).tolist()
