...
def augment_dataset(dataset, transform, num_samples):
    augmented_images = []
    augmented_labels = []
...
def run():
    ...
    OUTPUT_DIR=get_dir('solutions/matthew/v10', args.subfolder)
    ...
    val_test_transform = transforms.Compose([
        transforms.Resize((IMAGE_SIZE, IMAGE_SIZE)),  # Resize to the same size as training images
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
    ])
    ...
    augmented_images, augmented_labels = augment_dataset(train_dataset, blur_transform, 5000)
    
    train_dataset_extended = CGIARDataset(
        split='train',
        root_dir=DATA_DIR,
        transform=train_transform,
        initial_image_size=INITIAL_IMAGE_SIZE,
        additional_data=(augmented_images, augmented_labels)
    )

    # Split the dataset into training and validation sets
    train_size = int(0.8 * len(train_dataset))
    val_size = len(train_dataset) - train_size
    train_subset, val_subset = random_split(train_dataset_extended, [train_size, val_size])
    ...
    test_dataset = CGIARDataset(root_dir=DATA_DIR, split='test', transform=val_test_transform)
